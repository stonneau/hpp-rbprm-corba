...
def gen(limbs, num_samples = 1000, coplanar = True, num_candidates_per_config = 0, num_contact_candidates = 10, q_entries = None, projectToObstacles = False, unstable = False):
    ...
    for i in range(num_samples):
        if(q_entries == None):
            if(coplanar):
                q = fullBody.generateGroundContactFixed(q_half_sit, limbs)
            else:
...
res_pin = []
for i in range(10000):
	q = fullBody.generateGroundContactFixed(q_half_sit, [lLegId,rLegId])
	res+=[q]
	q_pin = q[:]
	quat_end = q_pin[4:7]
	q_pin[6] = q_pin[3]
...
from pickle import dump
#~ f1=open("configs_feet_on_ground_static_eq", 'w+')
f1=open("configs_statically_balanced_fixed_contacts", 'w+')
dump(res_pin, f1)
f1.close()
